...
void mark_coords(int x1, int y1, int x2, int y2, int coords[ARR_SIZE][ARR_SIZE]) {
	...
	if (x1 == x2) {
		printf("Processing vertical pairs: (%d, %d) -> (%d, %d)...", x1, y1, x2, y2);
		...
		printf("Processing horizontal pairs: (%d, %d) -> (%d, %d)...", x1, y1, x2, y2);
		...
		printf("Processing diagonal pairs: (%d, %d) -> (%d, %d)...", x1, y1, x2, y2);
		...
		do {
			...
			x1 += x_inc;
			y1 += y_inc;
		} while (x1 != (x_inc > 0 ? x2 + 1 : x2 - 1) || y1 != (y_inc > 0 ? y2 + 1 : y2 - 1));
